...
create policy "members manage sales customers" on public.sales_customers for all using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));
create policy "members manage sales products" on public.sales_products for all using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));
create policy "members manage sales proposals" on public.sales_proposals for all using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));
create policy "members manage proposal items" on public.sales_proposal_items for all using (exists(select 1 from public.sales_proposals p where p.id=proposal_id and public.is_organization_member(p.organization_id))) with check (exists(select 1 from public.sales_proposals p where p.id=proposal_id and public.is_organization_member(p.organization_id)));
